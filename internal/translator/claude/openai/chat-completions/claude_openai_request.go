...
func ConvertOpenAIRequestToClaude(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	if v := root.Get("reasoning_effort"); v.Exists() {
		...
		if effort != "" {
			...
			// Old behavior: treat Claude adaptive models that advertise a "max" level as max-capable.
			supportsMax := supportsAdaptive && hasThinkingLevel(mi.Thinking.Levels, "max")
			...
			if supportsAdaptive {
				...
				case "auto":
					...
					out, _ = sjson.DeleteBytes(out, "output_config.effort")
				default:
					if mapped, ok := mapToClaudeEffort(effort, supportsMax); ok {
						effort = mapped
					}
					out, _ = sjson.SetBytes(out, "thinking.type", "adaptive")
					out, _ = sjson.DeleteBytes(out, "thinking.budget_tokens")
